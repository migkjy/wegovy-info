...
import { clinics as staticClinics, REGIONS, type Region } from '@/data/clinics'
import { getClinicsList, getClinicRegions, type ClinicRow } from '@/lib/db/queries'
...
function normalizeStaticClinic(c: (typeof staticClinics)[number]): NormalizedClinic {
  return {
    id: c.id,
    name: c.name,
    region: c.region,
...
function ClinicCard({ clinic }: { clinic: NormalizedClinic }) {
  return (
    <Link
...
export default async function ClinicsPage({
  ...
  const { region: selectedRegion, q: searchQuery } = await searchParams

  const dbResult = await getClinicsList(0, 200, selectedRegion, searchQuery)
  const dbRegions = await getClinicRegions()

  const useDb = dbResult.items.length > 0
  let clinicList: NormalizedClinic[]
  let regions: string[]

  if (useDb) {
    clinicList = dbResult.items.map(normalizeDbClinic)
    regions = dbRegions.length > 0 ? dbRegions : [...REGIONS]
  } else {
    const filtered = selectedRegion
      ? staticClinics.filter((c) => c.region === selectedRegion)
      : searchQuery
        ? staticClinics.filter(
            (c) =>
              c.name.includes(searchQuery) || c.address.includes(searchQuery),
          )
        : staticClinics
    clinicList = filtered.map(normalizeStaticClinic)
    regions = [...REGIONS]
  }

  const grouped = regions.reduce<Record<string, NormalizedClinic[]>>(
    (acc, region) => {
      acc[region] = clinicList.filter((c) => c.region === region)
      return acc
    },
    {},
  )
    ...
    <>
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-lg px-4 py-3 text-sm text-amber-800 mb-6">
          <strong>안내:</strong> {PRICE_DISCLAIMER} 이 정보는 참고용이며, 처방 결정은 반드시
          전문의와 상담하세요.
        </div>

        {/* 검색 */}
        <form className="mb-6" action="/clinics" method="get">
        ...
        {clinicList.length === 0 ? (
          <div className="text-center py-16 text-gray-400">
            ...
            <p className="text-sm">다른 검색어나 지역을 시도해보세요.</p>
          </div>
        ) : selectedRegion ? (
          <section>
            <h2 className="text-lg font-semibold text-gray-800 mb-4">
              {selectedRegion} ({clinicList.length}개)
            </h2>
